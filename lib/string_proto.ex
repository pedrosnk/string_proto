defmodule StringProto do
  @moduledoc """
  Controls an IO device process that wraps a string.
  A `StringProto` IO device can be passed as a "device" to
  most of the functions in the `IO` module.
  ## Examples
      iex> {:ok, pid} = StringProto.open("foo")
      iex> IO.read(pid, 2)
      "fo"
  """
  ...
  ## Examples
      ...
      iex> IO.gets(pid, ">")
      "foo"
      iex> StringProto.contents(pid)
      {"", ""}
      iex> {:ok, pid} = StringProto.open("foo", capture_prompt: true)
      iex> IO.gets(pid, ">")
      "foo"
      iex> StringProto.contents(pid)
      {"", ">"}
  ...
  device.
  ## Examples
      iex> {:ok, pid} = StringProto.open("in")
