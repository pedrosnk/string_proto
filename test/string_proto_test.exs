defmodule StringProtoTest do
  use ExUnit.Case, async: true

  test "open and close" do
    {:ok, pid} = StringProto.open("")
    assert StringProto.close(pid) == {:ok, {"", ""}}
  end

  test "contents" do
    {:ok, pid} = StringProto.open("abc")
    IO.write(pid, "edf")
    assert StringProto.contents(pid) == {"abc", "edf"}
  end

  test "flush" do
    {:ok, pid} = StringProto.open("")
    IO.write(pid, "edf")
    assert StringProto.flush(pid) == "edf"
    assert StringProto.contents(pid) == {"", ""}
  end

  def start(string, opts \\ []) do
    StringProto.open(string, opts) |> elem(1)
  end

  def contents(pid) do
    StringProto.contents(pid)
  end
